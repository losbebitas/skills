...
| **Azure DevOps** | Azure Boards User Stories and Features | the `azdo-cli-axi` CLI |
...
"Other" is not a stub either. It is the reason Jira, Linear and Beads all work: you describe the workflow, the skill records your prose in `docs/agents/issue-tracker.md`, and the downstream skills follow the prose. The community has already done this: a Jira-over-[MCP](https://www.aihero.dev/ai-coding-dictionary/mcp) variant, a Gitea CLI shaped like `gh`, a hand-built local dashboard.
